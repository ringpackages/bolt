...
class Bolt {
    ...
    /// @brief Adds a default homepage displaying server info and registered routes.
    /// @details Shows Bolt name, version, and description.
    ///          Only adds the route if "/" is not already defined.
    /// @return Self for chaining.
    func homepage() {
        ...
        @get("/", func {
            ...
            aData = [
                ...
                :developer = aPackageInfo[:developer],
                :has_docs = lHasDocs
            ]
            cHtml = $bolt.renderTemplate($bolt_homepage_template, aData)
            $bolt.html(cHtml)
        })
